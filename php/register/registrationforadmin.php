...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT fullname, email, username FROM bcp_sms4_admins WHERE fullname = ? OR email = ? OR username = ?";
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO bcp_sms4_admins (user_type, username, fullname, password, email) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss", $type, $username, $fullname,$hashed_password, $email);
